...
class Generic_WSI_MTL_Dataset(Dataset):
	def __init__(self,
		...
		label_dicts = [{}, {}, {}],
		patient_strat=False,
		label_cols = ['MSI_label', 'TMB_label'],
		patient_voting = 'max',
		filter_dict = {},
		):
		"""
		...
		clinical_feature = slide_data.columns.tolist()[2:]
		self.label_cols = clinical_feature
		print(clinical_feature)
	...
	def cls_ids_prep(self):
		...
		for i in range(self.num_classes[0]):
			self.slide_cls_ids[i] = np.where(self.slide_data['MSI_label'] == i)[0]
	...
	def patient_data_prep(self, patient_voting='max'):
		...
		for p in patients:
			...
			label = self.slide_data['MSI_label'][locations].values
	...
	def df_prep(data, label_dicts, label_cols):
		if label_cols[0] != 'MSI_label':
			data['label'] = data[label_cols[0]].copy()

		data.reset_index(drop=True, inplace=True)
		for i in data.index:
			key = data.loc[i, 'MSI_label']
			data.at[i, 'MSI_label'] = label_dicts[0][key]
	...
	def getlabel(self, ids, task):
		if task > 0:
			...
			return self.slide_data['MSI_label'][ids]
...
class Generic_MIL_MTL_Dataset(Generic_WSI_MTL_Dataset):
	...
	def __getitem__(self, idx):
		...
		label = self.slide_data['MSI_label'][idx]
		site = self.slide_data[self.label_cols[1]][idx]
		gender = self.slide_data[self.label_cols[2]][idx]
		lymph = self.slide_data[self.label_cols[3]][idx]
		T = self.slide_data[self.label_cols[4]][idx]
		N = self.slide_data[self.label_cols[5]][idx]
		M = self.slide_data[self.label_cols[6]][idx]
		age = self.slide_data[self.label_cols[7]][idx]
		...
		if not self.use_h5:
			...
			return features, label, site,gender,lymph,T,N,M,age
			...
			return features, label, site,gender,lymph,T,N,M,age, coords
