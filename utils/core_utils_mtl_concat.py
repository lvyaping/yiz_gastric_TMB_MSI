...
def train(datasets, cur, args):
    ...
    # weight = [1, 5]
    weight = torch.tensor([1.,20.], device=device)
...
def train_loop(epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):   
    ...
    site_train_loss = 0.
    print('\n')
    for batch_idx, (data, label, site,gender,lymph,T,N,M,age) in enumerate(loader):
        data =  data.to(device)
        label = label.to(device)
        site = site.to(device)
        gender = gender.float().to(device)
        lymph = lymph.float().to(device)
        T = T.float().to(device)
        N = N.float().to(device)
        M = M.float().to(device)
        age = age.float().to(device)
        results_dict = model(data,gender,lymph,T,N,M,age)
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None):
    ...
    # print(true_, pred_, type(true_), type(pred_)) 
    # cls_val_recall = recall_score(true_, pred_)

    if early_stopping:
        assert results_dir
        early_stopping(epoch, loss_value_, model, ckpt_name = os.path.join(results_dir, "s_{}_checkpoint.pt".format(cur)))
