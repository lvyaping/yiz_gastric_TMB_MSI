...
def collate_MIL_mtl_concat(batch):
	...
	gender = torch.LongTensor([item[3] for item in batch])
	lymph = torch.LongTensor([item[4] for item in batch])
	T = torch.LongTensor([item[5] for item in batch])
	N = torch.LongTensor([item[6] for item in batch])
	M = torch.LongTensor([item[7] for item in batch])
	age = torch.LongTensor([item[8] for item in batch])
	return [img, label, site,gender,lymph,T,N,M,age]
